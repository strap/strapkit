...
module.exports = function info() {
    ...
    var output = '';
    if (!projectRoot) {
        return Q.reject(new Error('Current working directory is not a StrapKit-based project.'));
    }
    ...
    return Q.allSettled([
            ...
            //Get StrapKit version
            Q('StrapKit version: ' + package.version),
...
function getPlatforms(projectRoot) {
    var platforms = strapkit_util.listPlatforms(projectRoot);
    if (platforms.length) {
...
function listPlugins(projectRoot) {
    var pluginPath = path.join(projectRoot, 'plugins'),
        plugins    = strapkit_util.findPlugins(pluginPath);
...
function getProjectConfig(projectRoot) {
    ...
    return Q('Config.xml file: \n\n' + (fs.readFileSync(strapkit_util.projectConfig(projectRoot), 'utf-8')));
}
