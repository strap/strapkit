...
var config            = require('./config'),
    strapkit           = require('./strapkit'),
    strapkit_util      = require('./util'),
...
function add(hooks, projectRoot, targets, opts) {
    ...
    if (!targets || !targets.length) {
        return Q.reject(new StrapkitError('No platform specified. Please specify a platform to add. See "platform list".'));
    }
    var config_json = config.read(projectRoot);
    var platformsDir = path.join(projectRoot, 'platforms');

    // The "platforms" dir is safe to delete, it's almost equivalent to
    // strapkit platfrom rm <list of all platforms>
        ...
        return targets.reduce(function(soFar, t) {
            return soFar.then(function() {
                ...
                .then(function(libDir) {
                    ...
                    throw new StrapkitError('Unable to fetch platform ' + t + ': ' + err);
                });
            });
        }, Q());
    })
...
function remove(hooks, projectRoot, targets, opts) {
    if (!targets || !targets.length) {
        return Q.reject(new StrapkitError('No platform[s] specified. Please specify platform[s] to remove. See "platform list".'));
    }
...
function update(hooks, projectRoot, targets, opts) {
    ...
    if (!targets || !targets.length) {
        return Q.reject(new StrapkitError('No platform specified. Please specify a platform to update. See "platform list".'));
    } else if (targets.length > 1) {
        return Q.reject(new StrapkitError('Platform update can only be executed on one platform at a time.'));
    } else {
        var plat = targets[0];
        var platformPath = path.join(projectRoot, 'platforms', plat);
        var installed_platforms = strapkit_util.listPlatforms(projectRoot);
        if (installed_platforms.indexOf(plat) < 0) {
            return Q.reject(new StrapkitError('Platform "' + plat + '" is not installed. See "platform list".'));
        }

        function copyStrapKitJs() {
            ...
            shell.cp('-f', path.join(parser.www_dir(), 'strapkit.js'), path.join(platform_www, 'strapkit.js'));
        }
        ...
        .then(function() {
            ...
            .then(function() {
                // Copy the new strapkit.js from www -> platform_www.
                copyStrapKitJs();
...
function check(hooks, projectRoot) {
    var platformsText = [],
        platforms_on_fs = strapkit_util.listPlatforms(projectRoot),
        scratch = path.join(os.tmpdir(), "strapkit-platform-check-"+Date.now()),
        listeners = events._events;
        events._events = {};
    var result = Q.defer();
    strapkit.raw.create(scratch)
    .then(function () {
        ...
        Q.all(platforms_on_fs.map(function(p) {
            ...
            .then(function() {
                var d_avail = Q.defer(),
                    d_cur = Q.defer();
                getVersionFromScript(path.join(scratch, 'platforms', p, 'strapkit', 'version'), null)
                .then(function(avail) {
                    if (!avail) {
                ...
                getVersionFromScript(path.join(projectRoot, 'platforms', p, 'strapkit', 'version'), null)
                .catch(function () {
...
function list(hooks, projectRoot) {
    var platforms_on_fs = strapkit_util.listPlatforms(projectRoot);
    return hooks.fire('before_platform_ls')
    .then(function() {
        // Acquire the version number of each platform we have installed, and output that too.
        return Q.all(platforms_on_fs.map(function(p) {
            return getVersionFromScript(path.join(projectRoot, 'platforms', p, 'strapkit', 'version'), null)
            .then(function(v) {
                if (!v) return p;
                return p + ' ' + v;
            }, function(v) {
...
module.exports = function platform(command, targets) {
    ...
    if (targets) {
        ...
        targets.forEach(function(t) {
            if (!(t in platforms)) {
                err = new StrapkitError('Platform "' + t + '" not recognized as a core strapkit platform. See "platform list".');
            }
        });
        if (err) return Q.reject(err);
    } else {
        if (command == 'add' || command == 'rm') {
            return Q.reject(new StrapkitError('You need to qualify `add` or `remove` with one or more platforms!'));
        }
    }
...
function supports(project_root, name) {
    // required parameters
    if (!name) return Q.reject(new StrapkitError('requires a platform name parameter'));

    // check if platform exists
    var platform = platforms[name];
    if (!platform) {
        return Q.reject(new StrapkitError(util.format('"%s" platform does not exist', name)));
    }
...
function createOverrides(projectRoot, target) {
    shell.mkdir('-p', path.join(strapkit_util.appDir(projectRoot), 'merges', target));
};
...
function call_into_create(target, projectRoot, cfg, libDir, template_dir, opts) {
    ...
    if (fs.existsSync(output)) {
        return Q.reject(new StrapkitError('Platform ' + target + ' already added'));
    } else {
        ...
        .then(function() {
            ...
            if (target == 'android') {
                ...
                if (semver.gt(platformVersion, '0.1.0')) {
                    args.push('--cli');
                }
            } else if (target == 'ios') {
                var platformVersion = fs.readFileSync(path.join(libDir, 'StrapKitLib', 'VERSION'), 'UTF-8').trim();
                args.push('--arc');
                if (semver.gt(platformVersion, '0.1.0')) {
                    args.push('--cli');
                }
            ...
            .then(function() {
                ...
                var plugins = strapkit_util.findPlugins(plugins_dir);
