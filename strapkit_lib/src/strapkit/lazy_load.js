...
module.exports = {
    ...
    strapkit:function lazy_load(platform) {
        if (!(platform in platforms)) {
            return Q.reject(new Error('StrapKit library "' + platform + '" not recognized.'));
        }

        var url = platforms[platform].url + ';a=snapshot;h=' + platforms[platform].version + ';sf=tgz';
        return module.exports.custom(url, 'strapkit', platform, platforms[platform].version);
    },
    // Returns a promise for the path to the lazy-loaded directory.
    ...
    based_on_config:function(project_root, platform) {
        ...
        if (custom_path) {
            ...
            return module.exports.strapkit(platform);
        }
    }
};
