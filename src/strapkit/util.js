...
var global_config_path = path.join(HOME, '.strapkit');
...
function isRootDir(dir) {
    if (fs.existsSync(path.join(dir, 'js'))) {
        ...
        if (fs.existsSync(path.join(dir, 'js', 'config.xml'))) {
            return 1;
        }
    }
    return 0;
...
exports = module.exports = {
    ...
    // Runs up the directory chain looking for a .strapkit directory.
    // IF it is found we are in a StrapKit project.
    // Omit argument to use CWD.
    isStrapKit: function isStrapKit(dir) {
        // console.log(dir);
        if (!dir) {
            ...
            var cwd = process.cwd();
            if (pwd && pwd != cwd) {
                return this.isStrapKit(pwd) || this.isStrapKit(cwd);
            }
            return this.isStrapKit(cwd);
        }
        ...
        console.error('Hit an unhandled case in util.isStrapKit');
        return false;
    },
    // Cd to project root dir and return its path. Throw StrapkitError if not in a StrapKit project.
    cdProjectRoot: function() {
        var projectRoot = this.isStrapKit();
        if (!projectRoot) {
            throw new StrapkitError('Current working directory is not a StrapKit-based project.');
        }
    ...
    preProcessOptions: function (inputOptions) {
        ...
        if (projectPlatforms.length === 0) {
            throw new StrapkitError('No platforms added to this project. Please use `strapkit platform add <platform>`.');
        }
