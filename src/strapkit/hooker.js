...
module.exports = function hooker(root) {
    var r = util.isStrapKit(root);
    if (!r) throw new StrapkitError('Not a StrapKit project ("'+root+'"), can\'t use hooks.');
    else this.root = r;
};
...
module.exports.prototype = {
    ...
    fire:function fire(hook, opts) {
        ...
        return execute_handlers_serially(handlers, opts)
        .then(function() {
            return fireHooksInDir(path.join(root, '.strapkit', 'hooks', hook));
        }).then(function() {
...
function execute_scripts_serially(scripts, root, dir, opts) {
    ...
    if (scripts.length) {
        ...
        if (fs.statSync(fullpath).isDirectory()) {
            ...
            execOpts.env = {};
            execOpts.env.STRAPKIT_VERSION = require('../../package').version;
            execOpts.env.STRAPKIT_PLATFORMS = opts.platforms ? opts.platforms.join() : '';
            execOpts.env.STRAPKIT_PLUGINS = opts.plugins?opts.plugins.join():'';
            execOpts.env.STRAPKIT_HOOK = fullpath;
            execOpts.env.STRAPKIT_CMDLINE = process.argv.join(' ');
            ...
            .catch(function(err) {
                ...
                if (!isWindows && err.code == 'EACCES') {
                    ...
                    throw new StrapkitError('Hook failed with error code ' + err.code + ': ' + fullpath);
                }
            }).then(function() {
                return execute_scripts_serially(scripts, root, dir, opts);
            });
